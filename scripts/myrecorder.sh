...
MESSAGE=$2
if [ -z "${MESSAGE}" ]; then
  docker compose run --build --rm app myrecorder ${CMD} --no-yes --browser-kind remote
else
  docker compose run --build --rm app myrecorder ${CMD} --no-yes  --message "${MESSAGE}" --browser-kind remote
fi
